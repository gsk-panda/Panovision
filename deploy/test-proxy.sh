...
if curl -k -s -o /dev/null -w "%{http_code}" https://panorama.officeours.com/api/?type=log&log-type=traffic&key=test&nlogs=1 | grep -q "200\|401\|403"; then
    echo "✓ Can reach Panorama server"
else
    echo "✗ Cannot reach Panorama server"
    echo "  Testing DNS resolution..."
    nslookup panorama.officeours.com
    echo ""
    echo "  Testing connectivity..."
    ping -c 2 panorama.officeours.com 2>/dev/null || echo "  Ping failed"
fi
echo ""
...
echo "1. Verify Panorama server is accessible: curl -k https://panorama.officeours.com"
echo "2. Check Nginx can resolve DNS: nslookup panorama.officeours.com"
